...
const STRIPE_TOOL = "stripe.paymentIntents.create";
...
async function evaluateInProcess(input: {
  ...
  return res.json() as { decision: "allow" | "deny" };
}
...
describeIfDb("policy engine (Stage 6 exit)", () => {
  ...
  it("activating a new policy version changes proxy behavior without SDK redeploy", async () => {
    const mock = await startMockUpstream();
    const traceDeny = `trc_${randomUUID().replace(/-/g, "").slice(0, 24)}`;

    try {
      await expect(
        wrapFetch(mock.url, { method: "GET" }, {
          context: {
      ...
      const { policy } = await createPolicy(getPool(), DEV_ORG, null, {
        name: "Allow Stripe v2",
        rules: [
          {
            tool_pattern: STRIPE_TOOL,
            decision: "allow",
            priority: 200,
          },
        ],
      });
      await activatePolicy(getPool(), DEV_ORG, policy.policy_id);

      const evalAfter = await evaluateInProcess({
        organization_id: DEV_ORG,
        agent_id: DEV_AGENT,
        tool_name: STRIPE_TOOL,
      });
      expect(evalAfter.decision).toBe("allow");

      mock.getRequestCount(); // reset by tracking from 0 - need reset
      const countBefore = mock.getRequestCount();
      const traceAllow = `trc_${randomUUID().replace(/-/g, "").slice(0, 24)}`;
      const res = await wrapFetch(mock.url, { method: "GET" }, {
        context: {
          organizationId: DEV_ORG,
          agentId: DEV_AGENT,
          keyId: DEV_KEY,
      ...
      expect(mock.getRequestCount() - countBefore).toBe(1);
    } finally {
      mock.server.close();
    }
  });
