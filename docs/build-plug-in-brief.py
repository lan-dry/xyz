"""Generate Plug-in Venture discovery brief docx from markdown source."""
...
MD = os.path.join(HERE, "plug-in-venture-discovery-brief.md")
OUT = os.path.join(HERE, "plug-in-venture-discovery-brief.docx")
...
def add_rich_paragraph(doc: Document, text: str, *, quote: bool = False, bullet: bool = False) -> None:
    ...
    for part in parts:
        if not part:
            continue
        m = re.match(r"\*\*(.+?)\*\*", part)
        run = para.add_run(m.group(1) if m else part)
        run.font.size = Pt(11)
        if m:
            run.bold = True
        if quote:
...
def build() -> None:
    ...
    while i < len(lines):
        ...
        if line.startswith("> "):
            add_rich_paragraph(doc, line[2:].strip(), quote=True)
            i += 1
            continue

        if re.match(r"^\d+\.\s", line.strip()):
            text = re.sub(r"^\d+\.\s", "", line.strip())
            add_rich_paragraph(doc, text, bullet=False)
