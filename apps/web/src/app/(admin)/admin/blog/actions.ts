...
export async function createBlogPost(formData: FormData) {
  const session = await requireAdminPermissionSession("admin:cms:write");
  ...
  redirect(`/admin/blog/${encodeURIComponent(slug)}`);
}
...
export async function updateBlogPost(previousSlug: string, formData: FormData) {
  ...
  revalidatePath("/admin/blog");
  revalidatePath(`/admin/blog/${previousSlug}`);
  revalidatePath(`/admin/blog/${slug}`);
  redirect(`/admin/blog/${encodeURIComponent(slug)}?saved=1`);
}
