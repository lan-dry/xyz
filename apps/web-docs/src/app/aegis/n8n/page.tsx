...
export default function N8nOrchestratorsPage() {
  ...
  return (
    <>
      ...
      <div className="callout">
        <strong>Least effort:</strong> Enable Workflow Bridge once on an agent, then add{" "}
        <em>one</em> HTTP node at the end of the workflow (
        <code>POST /v1/aegis/workflows/runs/capture</code>). Salanor starts, signs steps, and closes
        the trace server-side — no private keys in n8n.
      </div>
      ...
      <h2>2. Per workflow (1 HTTP node)</h2>
      <CodeBlock
        lang="text"
        title="Shape"
        code={`Trigger
  → … your existing nodes (unchanged) …
  → Record in Aegis   POST /v1/aegis/workflows/runs/capture`}
      />

      <h3>Capture (start + steps + complete)</h3>
      <CodeBlock
        lang="bash"
        title="POST /v1/aegis/workflows/runs/capture"
        code={`curl -X POST ${api}/v1/aegis/workflows/runs/capture \\
  ...
  -d '{
    ...
    "execution": {
      ...
      <p>
        Use the Salanor template <code>examples/n8n/aegis-workflow-bridge.json</code>. Replace
        “Your workflow work” with your real steps; keep the single <strong>Record in Aegis</strong>{" "}
        HTTP node.
      </p>

      <h2>4. Verify</h2>
      <p>
        Open the <code>trace_url</code> from the capture response in{" "}
